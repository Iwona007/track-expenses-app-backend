...
//    private final WalletRepository walletRepository;
    private final WalletBusinessRepository walletBusinessRepository;
    ...
    public WalletDTO createWallet(@Valid CreateWalletDTO createWalletDTO) {
        ...
        Wallet savedWallet = walletBusinessRepository.save(wallet);
    ...
    public WalletDTO updateWallet(@Min(1) @NotNull Long id, @Valid UpdateWalletDTO dto) {
        Wallet wallet = walletBusinessRepository.findById(id)
                .orElseThrow(() -> {
    ...
    public List<WalletDTO> getWallets() {
        return walletBusinessRepository.findAllByOrderByNameAsc().stream()
    ...
    public void deleteWalletById(@Min(1) @NotNull Long id) {
        if (walletBusinessRepository.existsById(id)) {
            walletBusinessRepository.deleteById(id);
        } else {
            throw new AppRuntimeException(
                    ErrorCode.W003,
    ...
    public WalletDTO findById(@Min(1) @NotNull Long id) {
        Optional<Wallet> wallet = walletBusinessRepository.findById(id);
    ...
    public List<WalletDTO> findAllByNameLikeIgnoreCase(@NotBlank() @Length(max = 20) @Pattern(regexp = "[\\w ]+") String name) {
        ...
        try {
            listOfWalletDTO = walletBusinessRepository.findAllByNameLikeIgnoreCase(name)
                    .stream()
                    .map(walletModelMapper::mapWalletEntityToWalletDTO)
                    .toList();
